# -*- coding: utf-8 -*-
"""MIC Workshop 4 : Recurrent Neural Networks.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1oWTJKF8vj3Dfia7dB3PT_IDtcvwub-lN
...
class RNNCell(nn.Module): #lmao had to fix the buggy code (they just forgot some parens)
    ...
    def forward(self, input, hidden):
        ...
        w_ih = torch.matmul(torch.Tensor(Variable(torch.zeros(self.hidden_size))),  torch.Tensor(self.input_size))
        w_hh = torch.matmul(torch.Tensor(Variable(torch.zeros(self.hidden_size))),  torch.Tensor(Variable(torch.zeros(self.hidden_size))))
        b_ih = torch.Tensor(self.hidden_size)
        b_hh = torch.Tensor(self.hidden_size)
        hidden = Variable(torch.zeros(self.hidden_size))
        output = torch.tanh(torch.matmul(w_ih, input) + b_ih + torch.matmul(w_hh, self.hidden_size) + b_hh)
...
class CharRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, model, n_layers=1):
        ...
        self.encoder = nn.Embedding(input_size,hidden_size)   # is this right?
    ...
    def forward(self, input, hidden):
        ...
        encoded = self.encoder(input) #this is just some matrix operations
